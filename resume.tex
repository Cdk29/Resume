...
More precisly, the circular RNA design script have for mandatory input :
\begin{itemize}
	\item a file containing the names of the microRNAs that the user want the sponge to decoy from their original messenger RNA target. The names have to be the same than the one in the triplexRNA database. This file is a txt file with just the name of the microRNAs, one per line.
...
That means that, outside the seed region, where a strict complementarity is required, or in the additional 3' pairing region (cf above), the sequence is write with U or A. \\
