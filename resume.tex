...
The plot does not show a perfect separation of tumor from normal samples as it should be expected for a differential expression analysis which performed well (see figure below). Beside the scatterplot for the differential expression analysis performed on the set of genes inside the KEGG Melanoma pathways, is represented the scatterplot for a differential expression analysis perform on curated RNA-seq data for the colorectal cancer\cite{curated}.\\
